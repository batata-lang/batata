...
| `ex.term.enumerable_reduce` | `(enumerable: i64, acc: i64, continuation: i64) -> i64` | tag-dispatched reduce over list/tuple/binary/map; continuation 1 = sum (acc + item), 2 = return acc, 3 = map values sum, 4 = map keys sum, 5 = map entries sum (acc + key + value), 6 = product (acc * item) |
