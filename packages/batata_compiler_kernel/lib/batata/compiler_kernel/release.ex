defmodule Batata.CompilerKernel.Release do
  ...
  def build!(output_dir, ctx, opts)
    ...
    index = release_index(stage1, stage2, opts)
    index_path = Path.join(output_dir, "release-index.json")
    File.write!(index_path, Memory.canonical_json(index) <> "\n")

    %{index: index, index_path: index_path, stage1: stage1, stage2: stage2}
  end
  ...
  defp validate_options!(opts) do
    ...
    allowed = @required_options ++ [:beaver_path]
  ...
  defp release_index(stage1, stage2, opts) do
    %{
      "schema_version" => 1,
