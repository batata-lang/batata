defmodule Batata.ObjC.Diagnostic do
  @moduledoc "A stable, machine-readable Objective-C binding failure."
  ...
  def exception(options) do
    ...
    unless is_binary(code) and String.starts_with?(code, "E_OBJC_") do
      raise ArgumentError, "Objective-C diagnostic code must start with E_OBJC_"
    end

    unless is_binary(message) and is_map(context) and is_list(actions) do
      raise ArgumentError, "invalid Objective-C diagnostic fields"
    end
  ...
  @doc "Converts the diagnostic to a JSON-ready map with stable string keys."
  @spec to_map(t()) :: map()
