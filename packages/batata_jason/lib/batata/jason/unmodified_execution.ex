defmodule Batata.Jason.UnmodifiedExecution do
  ...
  defp beam_oracle!(files) do
    ...
    try do
      ...
      encoded
      |> Base.decode64!()
      # The bytes come only from the fixed wrapper and pinned sources compiled
      # by the child process above. Its fixture module atoms intentionally do
      # not exist in this VM yet, so the safe decoder cannot materialize them.
