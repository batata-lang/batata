defmodule Batata.Probe.Jason.CapabilityMatrixTest do
  ...
  test "tracks executable semantics separately from source inventory counts" do
    ...
    assert capabilities["stdlib.maps_from_list"]["status"] == "executable"
    assert capabilities["stdlib.maps_from_list"]["gate"] == "stdlib_test"
    assert capabilities["stdlib.maps_from_list"]["scope"] =~ "duplicate-key last-value"
    ...
    assert capabilities["control.short_circuit_or"]["status"] == "executable"
    assert capabilities["control.short_circuit_or"]["gate"] == "execute_test"

    assert capabilities["control.short_circuit_or"]["scope"] =~
             "right-hand-side assignments excluded"

    assert capabilities["control.strict_boolean_and"]["status"] == "executable"
    assert capabilities["control.strict_boolean_and"]["gate"] == "execute_test"
    assert capabilities["control.strict_boolean_and"]["scope"] =~ "scalar-refined variables"
    assert capabilities["control.strict_boolean_and"]["scope"] =~ "BadBooleanError excluded"
    ...
    assert capabilities["exception.execution"]["scope"] =~ "Jason.DecodeError"
    assert capabilities["exception.execution"]["scope"] =~ "Protocol.UndefinedError"
    ...
    assert capabilities["jason.unmodified"]["scope"] =~ "20 deterministic"
