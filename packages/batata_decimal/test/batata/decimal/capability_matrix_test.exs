defmodule Batata.Probe.Decimal.CapabilityMatrixTest do
  ...
  test "marks aggregate numeric guards executable only with both sub-capabilities" do
    ...
    assert capabilities["decimal.unmodified_finite"]["scope"] =~ "23 BEAM-oracle cases"
    ...
    assert capabilities["decimal.bigint_arithmetic_comparison"]["status"] == "executable"

    assert capabilities["decimal.bigint_arithmetic_comparison"]["gate"] ==
             "unmodified_execution"

    assert capabilities["decimal.bigint_arithmetic_comparison"]["scope"] =~
             "adjacent comparison"
  end
end
