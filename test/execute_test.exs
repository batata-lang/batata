defmodule Batata.ExecuteTest do
  ...
  test "evaluates each qualified Kernel.max/2 operand exactly once", %{ctx: ctx} do
    source = """
    defmodule ScalarMaxEvaluation do
      ...
      def main() do
        result = Kernel.max(tap(3, 3), tap(5, 5))
  ...
  test "executes an unprotected integer BIF in a term-pattern guard", %{ctx: ctx} do
    assert 1 ==
             Batata.execute(
               """
               defmodule Math do
